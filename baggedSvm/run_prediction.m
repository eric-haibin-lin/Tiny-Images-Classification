...
Ms = [3, 5, 7, 9, 71, 13, 15, 17, 19, 21, 23, 27];
...
%for mIndex = 1:numberofMs
    ...
    for index = 1:M
        ...
        model = svmtrain(baggedTTrain, baggedXTrain, '-t 1');
        [predict_labelTest, accuracyTest, dec_valuesTest] = svmpredict(TTest, XTest, model); 

        labelTestArr(:, index) = predict_labelTest;

    end 

    finalLabel = mode(labelTestArr, 2);
